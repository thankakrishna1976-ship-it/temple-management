...
NATCHATHIRAM_LIST = [
    ...
    'ரோகிணி (Rohini)', 'மிருகசீரி��ம் (Mrigashira)', 'திருவாதிரை (Ardra)',
    ...
    'மூ��ம் (Mula)', 'பூராடம் (Purva Ashadha)', 'உத்திராடம் (Uttara Ashadha)',
            ...
            for key, value in filters.items():
                if isinstance(value, dict):
                    for op, val in value.items():
                        ...
                        elif op == 'like': query = query.like(key, val)
                        elif op == 'ilike': query = query.ilike(key, val)
                else:
...
def init_session():
    defaults = {
        ...
        'view_bill_id': None, 'edit_samaya_id': None, 'edit_mandapam_id': None,
    }
...
def page_dashboard():
    ...
    today_d = date.today()
    if period == 'Daily': sd, ed = today_d, today_d
    elif period == 'Weekly': sd, ed = today_d - timedelta(days=today_d.weekday()), today_d
    elif period == 'Monthly': sd, ed = today_d.replace(day=1), today_d
    else: sd, ed = today_d.replace(month=1, day=1), today_d

    # Income
    bills = db_select("bill", "amount", filters={"is_deleted": False})
    period_bills = [b for b in bills
                    if sd.isoformat() <= str(b.get('bill_date', ''))[:10] <= ed.isoformat()]
    total_income = sum(float(b.get('amount', 0) or 0) for b in period_bills)

    # Expenses
    expenses = db_select("expense", "amount,expense_date")
    period_exp = [e for e in expenses
                  if sd.isoformat() <= str(e.get('expense_date', ''))[:10] <= ed.isoformat()]
    total_expenses = sum(float(e.get('amount', 0) or 0) for e in period_exp)

    # Counts
    all_devotees = db_select("devotee", "id", filters={"is_family_head": True, "is_active": True})
    total_devotees = len(all_devotees)
    total_bills = len(period_bills)

    # Cards
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        st.markdown(f'<div class="stat-card income"><h4>⬆️ Income</h4><h2>₹{total_income:,.2f}</h2></div>', unsafe_allow_html=True)
    with c2:
        st.markdown(f'<div class="stat-card expense"><h4>⬇️ Expenses</h4><h2>₹{total_expenses:,.2f}</h2></div>', unsafe_allow_html=True)
    with c3:
        st.markdown(f'<div class="stat-card devotees"><h4>👥 Devotees</h4><h2>{total_devotees}</h2></div>', unsafe_allow_html=True)
    with c4:
        st.markdown(f'<div class="stat-card bills"><h4>🧾 Bills</h4><h2>{total_bills}</h2></div>', unsafe_allow_html=True)

    # Birthdays
    all_devs = db_select("devotee", "name,mobile_no,dob", filters={"is_active": True})
    todays_bdays = [d for d in all_devs if d.get('dob') and str(d['dob'])[5:10] == today_d.strftime('%m-%d')]
    if todays_bdays:
        bday_text = " | ".join([f"🎂 Happy Birthday {b['name']}!" for b in todays_bdays])
        st.markdown(f'<div class="news-ticker">{bday_text}</div>', unsafe_allow_html=True)
    ...
    with cr:
        ...
        if todays_bdays:
            for b in todays_bdays:
                st.markdown(f"🎂 **{b['name']}** — {b.get('mobile_no') or '-'}")
        else:
...
def page_devotees():
    ...
    if devotees:
        for d in devotees:
            family = db_select("devotee", "id", filters={"family_head_id": d['id']})
            with st.container():
                c1, c2, c3, c4, c5 = st.columns([0.5, 2.5, 2, 1, 2])
                with c1: st.write(f"#{d['id']}")
                with c2: st.write(f"**{d['name']}**")
                with c3: st.write(f"📱 {d.get('mobile_no') or '-'}")
                with c4: st.write(f"👨‍👩‍👧 {len(family)}")
                with c5:
...
def page_add_devotee():
    ...
    st.markdown(f"## {'✏��� Edit' if dev else '➕ Add'} Devotee")
    ...
    with st.form("dev_form"):
        ...
        with c1:
            ...
            if dev and dev.get('dob'):
                try: dob_val = datetime.strptime(str(dev['dob'])[:10], '%Y-%m-%d').date()
                except: pass
        ...
        with c2:
            ...
            if dev and dev.get('wedding_day'):
                try: wed_val = datetime.strptime(str(dev['wedding_day'])[:10], '%Y-%m-%d').date()
                except: pass
        ...
        if st.form_submit_button("💾 Save", use_container_width=True):
            if not name.strip():
                ...
                data = {
                    ...
                    "mobile_no": mobile, "whatsapp_no": whatsapp,
                    "wedding_day": wedding.isoformat() if wedding else None,
                    "natchathiram": natch or None, "address": address,
    ...
    if cid:
        ...
        for fm in family:
            ...
            with c1: st.write(f"**{fm['name']}** ({fm.get('relation_type') or '-'})")
            with c2: st.write(f"DOB: {str(fm.get('dob') or '-')[:10]} | 📱 {fm.get('mobile_no') or '-'}")
            with c3:
                if st.button("🗑", key=f"rf{fm['id']}"):
                    db_delete("devotee", {"id": fm['id']}); st.rerun()
        ...
        with st.form("add_fm"):
            ...
            with fc3:
                fm = st.text_input("Mobile", key="fmo")
            if st.form_submit_button("➕ Add Member"):
                if fn.strip():
                    db_insert("devotee", {
                        "name": fn.strip(), "dob": fd.isoformat() if fd else None,
                        "relation_type": fr or None, "natchathiram": fs or None,
                        "mobile_no": fm, "family_head_id": cid,
                        "is_family_head": False, "is_active": True
                    })
                    st.rerun()
        ...
        for yp in yearly:
            c1, c2, c3 = st.columns([3, 3, 1])
            with c1: st.write(f"**{yp.get('pooja_name') or '-'}**")
            with c2: st.write(f"Date: {str(yp.get('pooja_date') or '-')[:10]} | {yp.get('notes') or '-'}")
            with c3:
                if st.button("🗑", key=f"ry{yp['id']}"):
                    db_delete("devotee_yearly_pooja", {"id": yp['id']}); st.rerun()
            ...
            if st.form_submit_button("➕ Add Pooja"):
                if yps:
                    ...
                    db_insert("devotee_yearly_pooja", {
                        "devotee_id": cid, "pooja_type_id": pt['id'],
                        "pooja_name": pt['name'], "pooja_date": ypd.isoformat(),
                        "notes": ypn
                    })
                    st.rerun()
...
def page_view_devotee():
    ...
    if not did: go_to('devotees'); return
    r = db_select("devotee", filters={"id": did})
    if not r: st.error("Not found!"); return
    d = r[0]
...
def page_billing():
    ...
    with fc1: fd = st.date_input("From", value=date.today(), format="DD/MM/YYYY", key="bf")
    with fc2: td = st.date_input("To", value=date.today(), format="DD/MM/YYYY", key="bt")
    ...
    if bills:
        for b in bills:
            ...
            with c1: st.write(f"**{b['bill_number']}**")
            with c2: st.write(str(b.get('bill_date',''))[:10])
            with c3: st.write(bname)
            with c4: st.write(f"**₹{float(b.get('amount',0)):,.2f}**")
            with c5:
                ...
                if is_admin() and st.button("🗑", key=f"db{b['id']}"):
                    db_update("bill", {
                        ...
                        "deleted_at": datetime.now().isoformat(), "delete_reason": "Admin deleted"
                    }, {"id": b['id']})
                    st.rerun()
            st.markdown("---")
    else:
...
def page_new_bill():
    ...
    with st.form("bill_form"):
        ...
        with c1: st.text_input("Bill No", value=nbn, disabled=True)
        with c2: manual = st.text_input("Manual Bill No")
        with c3: bdate = st.date_input("Date", value=date.today(), format="DD/MM/YYYY")
        ...
        if st.form_submit_button("💾 Create Bill", use_container_width=True):
            if not spt:
                ...
                result = db_insert("bill", {
                    "bill_number": nbn, "manual_bill_no": manual, "bill_book_no": "",
                    "bill_date": datetime.combine(bdate, datetime.now().time()).isoformat(),
                    "devotee_type": dtype.lower(), "devotee_id": dev_id,
                    "guest_name": guest_name, "guest_address": guest_address,
                    "guest_mobile": guest_mobile, "guest_whatsapp": guest_whatsapp,
                    "pooja_type_id": pt['id'], "amount": amount, "notes": notes,
    ...
    if st.button("⬅️ Back"): go_to('billing')


def page_view_bill():
    bid = st.session_state.get('view_bill_id')
    if not bid: go_to('billing'); return
    ...
    st.markdown(f"""<div class="bill-receipt">
        ...
        <table style="width:100%;margin-bottom:15px;">
            <tr><td><b>Bill:</b> {b['bill_number']}</td><td><b>Manual:</b> {b.get('manual_bill_no') or '-'}</td>
                <td style="text-align:right;"><b>Date:</b> {bdt}</td></tr>
            <tr><td><b>Name:</b> {bname}</td><td><b>Mobile:</b> {bmobile}</td>
        ...
        <table style="width:100%;border-collapse:collapse;">
            ...
            <tfoot><tr style="border-top:2px solid #8B0000;">
                ...
                <td style="padding:8px;text-align:right;color:#8B0000;font-size:1.3em;"><b>₹{amt:,.2f}</b></td></tr></tfoot></table>
...
def page_expenses():
    ...
    fc1, fc2 = st.columns(2)
    with fc1: fd = st.date_input("From", value=date.today().replace(day=1), format="DD/MM/YYYY", key="ef")
    with fc2: td = st.date_input("To", value=date.today(), format="DD/MM/YYYY", key="et")
    ...
    if exps:
        import pandas as pd
        for e in exps: e['type'] = et_map.get(e.get('expense_type_id'), '-')
        ...
        if did > 0 and st.button("🗑 Delete"):
            db_delete("expense", {"id": did}); st.rerun()
    ...
    with st.form("add_exp"):
        ...
        with ec2: ea = st.number_input("Amount *", min_value=0.0, step=10.0)
        with ec3: ed = st.date_input("Date", value=date.today(), format="DD/MM/YYYY")
        edesc = st.text_input("Description")
        if st.form_submit_button("💾 Save"):
            if ea > 0 and ets:
                eid = next(e['id'] for e in ets if e['name'] == se)
                db_insert("expense", {"expense_type_id": eid, "amount": ea,
                    "description": edesc, "expense_date": ed.isoformat(),
...
def page_reports():
    st.markdown("## 📈 Reports")
    fc1, fc2 = st.columns(2)
    with fc1: fd = st.date_input("From", value=date.today().replace(day=1), format="DD/MM/YYYY", key="rf")
    with fc2: td = st.date_input("To", value=date.today(), format="DD/MM/YYYY", key="rt")
    ...
    c1, c2, c3 = st.columns(3)
    with c1: st.markdown(f'<div class="stat-card income"><h4>Income</h4><h2>₹{ti:,.2f}</h2></div>', unsafe_allow_html=True)
    with c2: st.markdown(f'<div class="stat-card expense"><h4>Expenses</h4><h2>₹{te:,.2f}</h2></div>', unsafe_allow_html=True)
    with c3: st.markdown(f'<div class="stat-card {"income" if ti-te>=0 else "expense"}"><h4>Net</h4><h2>₹{ti-te:,.2f}</h2></div>', unsafe_allow_html=True)


# ============================================================
# SAMAYA, MANDAPAM, DAILY POOJA, SETTINGS, USERS
# ============================================================
def page_samaya():
    st.markdown("## 🎓 Samaya Vakuppu")
    if st.button("➕ Add"): go_to('add_samaya', edit_samaya_id=None)
    for s in db_select("samaya_vakuppu", order="student_name"):
        c1, c2, c3 = st.columns([3, 3, 1])
        with c1: st.write(f"**{s['student_name']}** | Parent: {s.get('father_mother_name') or '-'}")
        with c2: st.write(f"Bond: {s.get('bond_no') or '-'} | Bank: {s.get('bond_issuing_bank') or '-'}")
        with c3:
            if st.button("✏️", key=f"es{s['id']}"): go_to('add_samaya', edit_samaya_id=s['id'])
            if st.button("🗑", key=f"ds{s['id']}"): db_delete("samaya_vakuppu", {"id": s['id']}); st.rerun()
        st.markdown("---")
...
def page_add_samaya():
    ...
    with st.form("sf"):
        ...
        if st.form_submit_button("💾 Save"):
            if nm.strip():
                data = {"student_name": nm, "father_mother_name": par, "bond_no": bn,
                        "bond_issuing_bank": bk, "branch_of_bank": br, "address": addr}
                if eid and s: db_update("samaya_vakuppu", data, {"id": eid})
                else: db_insert("samaya_vakuppu", data)
                st.success("Saved! ✅"); go_to('samaya')
    if st.button("⬅️ Back"): go_to('samaya')


def page_mandapam():
    st.markdown("## 🏛️ Thirumana Mandapam")
    if st.button("➕ Add"): go_to('add_mandapam', edit_mandapam_id=None)
    for m in db_select("thirumana_mandapam", order="name"):
        c1, c2, c3 = st.columns([3, 3, 1])
        with c1: st.write(f"**{m['name']}** | Bond: {m.get('bond_no') or '-'}")
        with c2: st.write(f"₹{float(m.get('amount',0)):,.2f} | Bonds: {m.get('no_of_bond',1)}")
        with c3:
            if st.button("✏️", key=f"em{m['id']}"): go_to('add_mandapam', edit_mandapam_id=m['id'])
            if st.button("🗑", key=f"dm{m['id']}"): db_delete("thirumana_mandapam", {"id": m['id']}); st.rerun()
        st.markdown("---")
...
def page_add_mandapam():
    ...
    with st.form("mf"):
        ...
        with c2:
            ...
            nb = st.number_input("No of Bonds", value=int(m.get('no_of_bond',1)) if m else 1, min_value=1)
        addr = st.text_area("Address", value=m.get('address') or '' if m else '')
        if st.form_submit_button("💾 Save"):
            if nm.strip():
                data = {"name": nm, "bond_no": bn, "amount": amt, "no_of_bond": nb, "address": addr}
                if eid and m: db_update("thirumana_mandapam", data, {"id": eid})
                else: db_insert("thirumana_mandapam", data)
                st.success("Saved! ✅"); go_to('mandapam')
    if st.button("⬅️ Back"): go_to('mandapam')
...
def page_daily_pooja():
    ...
    for p in db_select("daily_pooja", filters={"is_active": True}, order="pooja_time"):
        c1, c2, c3 = st.columns([3, 3, 1])
        with c1: st.write(f"**{p['pooja_name']}** — {p.get('pooja_time') or 'TBD'}")
        with c2: st.write(p.get('description') or '-')
        with c3:
            if st.button("🗑", key=f"dp{p['id']}"):
                db_update("daily_pooja", {"is_active": False}, {"id": p['id']}); st.rerun()
    with st.form("adp"):
        c1, c2, c3 = st.columns(3)
        with c1: pn = st.text_input("Name *")
        with c2: pt = st.text_input("Time")
        with c3: pd = st.text_input("Description")
        if st.form_submit_button("➕ Add"):
            if pn.strip():
                db_insert("daily_pooja", {"pooja_name": pn, "pooja_time": pt, "description": pd, "is_active": True})
                st.rerun()
...
def page_settings():
    ...
    with cl:
        ...
        for p in db_select("pooja_type", filters={"is_active": True}, order="name"):
            ...
            with c1: st.write(p['name'])
            with c2: st.write(f"₹{p['amount']}")
            with c3:
                if st.button("🗑", key=f"dp{p['id']}x"):
                    db_update("pooja_type", {"is_active": False}, {"id": p['id']}); st.rerun()
        with st.form("apt"):
            pn = st.text_input("Name *", key="npn")
            pa = st.number_input("Amount", value=0.0, step=10.0, key="npa")
            if st.form_submit_button("➕ Add"):
                if pn.strip(): db_insert("pooja_type", {"name": pn, "amount": pa, "is_active": True}); st.rerun()
    with cr:
        st.markdown("### 🏷️ Expense Types")
        for e in db_select("expense_type", filters={"is_active": True}, order="name"):
            c1, c2 = st.columns([4, 1])
            with c1: st.write(e['name'])
            with c2:
                if st.button("🗑", key=f"de{e['id']}x"):
                    db_update("expense_type", {"is_active": False}, {"id": e['id']}); st.rerun()
        with st.form("aet"):
            en = st.text_input("Name *", key="nen")
            if st.form_submit_button("➕ Add"):
                if en.strip(): db_insert("expense_type", {"name": en, "is_active": True}); st.rerun()


def page_users():
    if not is_admin(): st.error("Admin only!"); return
    st.markdown("## 👤 Users")
    for u in db_select("users", order="id"):
        c1, c2, c3, c4 = st.columns([2, 2, 1, 1])
        with c1: st.write(f"**{u['username']}** ({u.get('full_name') or '-'})")
        ...
        with c3:
            if u['id'] != st.session_state['user_id']:
                if st.button("Toggle", key=f"tu{u['id']}"):
                    db_update("users", {"is_active_user": not u['is_active_user']}, {"id": u['id']}); st.rerun()
        st.markdown("---")
    with st.form("au"):
        st.markdown("### ➕ Add User")
        c1, c2 = st.columns(2)
        with c1: un = st.text_input("Username *"); fn = st.text_input("Full Name")
        with c2: pw = st.text_input("Password *", type="password"); rl = st.selectbox("Role", ['user','admin'])
        if st.form_submit_button("👤 Create"):
            if un and pw:
                if db_select("users", filters={"username": un}): st.error("Exists!")
                else:
                    db_insert("users", {"username": un, "password_hash": generate_password_hash(pw),
                        "full_name": fn, "role": rl, "is_active_user": True})
    ...
    if bills:
        ...
        for b in bills:
            if b.get('deleted_by'):
                ...
                b['deleted_by_name'] = u[0]['full_name'] if u else '-'
            else:
                b['deleted_by_name'] = '-'
        df = pd.DataFrame(bills)
        df['Date'] = df['bill_date'].apply(lambda x: str(x)[:10])
        df['Amount'] = df['amount'].apply(lambda x: f"₹{float(x or 0):,.2f}")
        st.dataframe(df[['bill_number','Date','Amount','deleted_by_name','delete_reason']].rename(
            columns={'bill_number':'Bill','deleted_by_name':'Deleted By','delete_reason':'Reason'}),
            use_container_width=True, hide_index=True)
    else:
...
def main():
    ...
    func = pages.get(page, page_dashboard)
    func()
